...
use crate::{structs::{Grade, Class, Day, SchoolData, self, RawSchoolDataKey, School, RawSchoolData, SchoolList}};
use fancy_regex::Regex;
use serde_json::Value;

pub fn view(school: &School, keys: RawSchoolDataKey) -> Result<SchoolData, Box<dyn std::error::Error>> {
    ...
    let request = format!("http://comci.kr:4082/{}?{}", &target, &encoded);
    let mut response = reqwest::blocking::get(request)?;
...
pub fn search_school(school: &'static str, keys: RawSchoolDataKey) -> Result<Vec<School>, Box<dyn std::error::Error>> {
...
pub fn request_target() -> Result<RawSchoolDataKey, Box<dyn std::error::Error>> {
    let request = "http://comci.kr:4082/st";
    let mut response = reqwest::blocking::get(request)?;
